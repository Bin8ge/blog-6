...
function settlement() {
    ...
    if (count == 0) {
        ...
        if (money != 0) {
            var req = {
                address_id:$(".cart-prolist select[name=address]").val(),
                items: [],
                remark:$(".cart-prolist textarea[name=order_items]").val(),
            };
            var i = 0;
            ...
            $(".cart-list-li").each(function() {
                if ($(this).find(".checkbox").attr("is_check") == 'yes') {
                    ...
                    var sku_id = $(this).find("input[name='quantity']").attr("data-sku_id");
                    var amounts = $(this).find("input[name='quantity']").val();
